...
SPECS={'work_plan_analyses':('plano_trabalho_analise_especial','id_analise_plano_trabalho'),'pending_organs':('orgao_analise_pendente_especial','id_orgao_analise_pendente')}
      ...
      try:
        while True:
          ...
          for row in batch:
            ...
            existing[str(row[key])]={'source':'Transferegov - Transferências Especiais','source_record_id':str(row[key]),'id_plano_trabalho':row.get('id_plano_trabalho'),**row,'source_url':r.url,'fetched_at':r.fetched_at,'sha256':r.sha256}
