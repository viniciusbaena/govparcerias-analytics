const S={data:null,contract:null,portfolio:null,contracts:[],view:'inicio',homeQuery:'',theme:localStorage.getItem('gpa:theme')||'dark',selectedSection:'identificacao',selectedMunicipality:null,municipalityQuery:'',municipalityPage:1,pageSize:18};
...
async function boot(){
  ...
  [S.data,S.contract,S.portfolio,S.contracts]=await Promise.all([
...
function inicio(){
  ...
  $('#content').innerHTML=
    head('Consulta unificada','Pesquise em um único campo por município, CNPJ, código IBGE, número do contrato, identificador PNCP, processo, objeto, fornecedor ou órgão.')
    +`<div class="home-search-grid">
      <article class="home-search-card">
        <span class="eyebrow">Busca integrada</span>
        <h2>Consultar contratos e municípios</h2>
        <p>A pesquisa combina a carteira dos 121 municípios com os contratos oficiais já sincronizados.</p>
        ...
        <small>Resultados contratuais são exibidos somente quando existem na base oficial sincronizada.</small>
      </article>
      <article class="home-search-card ai-home-card">
        ...
        <p>Use linguagem natural para consultar contratos e municípios já disponíveis na base.</p>
    ...
    <div class="metric-grid">
      ...
      ${card('Consulta unificada','Ativa','Município, CNPJ, IBGE, contrato, processo e objeto')}
      ${card('Integridade','Official-only','Sem valores ou registros inventados')}
    </div>
    <div class="truth-banner"><strong>Separação de confiança</strong><span>Municípios da carteira e contratos oficiais permanecem identificados por sua proveniência.</span></div>`;
...
function askHomeAI(){
  ...
  if(contracts.length||municipalities.length){
    const searchTerm=terms.join(' ')||q;
    box.innerHTML+=`<div class="notice"><strong>GovParcerias AI:</strong> Encontrei ${contracts.length} contrato(s) oficial(is) e ${municipalities.length} município(s) relacionados. <button id="aiViewResults">Ver resultados</button></div>`;
...
function municipalityProfile(){const m=S.selectedMunicipality;if(!m)return show('municipios');$('#content').innerHTML=head(m.name,'Visão municipal 360° com acesso aos contratos e consolidações territoriais.',`<button onclick="show('municipios')">← Voltar à carteira</button>`)+`<div class="profile-head"><div class="profile-mark">${esc(m.name.slice(0,2).toUpperCase())}</div><div><span class="eyebrow">Município da carteira</span><h2>${esc(m.name)}</h2><p>CNPJ ${m.cnpj} · Código IBGE ${m.ibge_code}</p></div><span class="source-pill">Carteira de trabalho</span></div><div class="metric-grid">${card('Contratos oficiais','—','Fonte ainda não sincronizada')}${card('Obras oficiais','—','Fonte ainda não sincronizada')}${card('Volume financeiro','—','Sem valor inventado')}${card('Alertas verificáveis','—','Motor aguarda evidências')}</div><div class="tabs-inline"><button onclick="showMunicipalityTab('contratos')">Contratos</button><button onclick="showMunicipalityTab('obras')">Obras</button><button onclick="showMunicipalityTab('financeiro')">Financeiro</button><button onclick="showMunicipalityTab('documentos')">Documentos</button></div><div id="municipalityTab">${empty('Nenhum contrato oficial vinculado','O município já está cadastrado na carteira. A associação com contratos ocorrerá por chaves oficiais após a sincronização das fontes públicas.')}</div><div class="provenance-box"><strong>Proveniência do cadastro</strong><p>Arquivo: ${esc(m.source_label)} · linha ${m.source_row} · classificação: carteira administrativa. O cadastro não é apresentado como validação pública oficial.</p></div>`}
function showMunicipalityTab(t){const names={contratos:'contrato',obras:'obra',financeiro:'lançamento financeiro',documentos:'documento'};$('#municipalityTab').innerHTML=empty(`Nenhum ${names[t]} oficial vinculado`,'A plataforma não criará registros sintéticos para preencher esta área.')}
...
function instrumentos(){
  ...
  const summary=query
    ?`${contracts.length} contrato(s) oficial(is) e ${municipalities.length} município(s) encontrado(s) para “${esc(query)}”.`
    :`${contracts.length} contrato(s) oficial(is) sincronizado(s) e ${municipalities.length} município(s) cadastrados.`;

  $('#content').innerHTML=
    head('Consulta unificada','Pesquise por município, CNPJ, IBGE, contrato, identificador PNCP, processo, objeto, fornecedor ou órgão.',`<button id="backToHomeButton">← Voltar</button>`)
    +`<div class="toolbar">
      ...
      <div class="portfolio-meta"><span>${summary}</span><span>Contratos carregados: ${(S.contracts||[]).length}</span><span>Modo: official-only</span></div>
      ...
      ${!contracts.length&&!municipalities.length?empty('Nenhum resultado localizado',`A consulta por ${esc(query)} não encontrou contratos sincronizados nem municípios da carteira.`):''}`;
...
Object.assign(window,{show,selectSection,openChat,closeChat,openPalette,closePalette,assistantBlocked,applyMunicipalitySearch,changeMunicipalityPage,openMunicipality,showMunicipalityTab,S});boot();
