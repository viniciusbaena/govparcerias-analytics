const APP_VERSION='v3.21.0-alpha';
...
const nav=[['inicio','Visão integrada'],['dashboard','Dashboard gerencial'],['municipios','Municípios'],['territorial','Gestão territorial'],['instrumentos','Contratos e propostas'],['dossie','Dossiê por contrato'],['financeiro','Inteligência financeira'],['engenharia','Obras e engenharia'],['documentos','Centro de documentos'],['timeline','Timeline integrada'],['riscos','Risco e conformidade'],['copiloto','Copiloto']];
...
function show(v){S.view=v;document.querySelectorAll('[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===v));({inicio,dashboard,municipios,territorial,instrumentos,dossie,financeiro,engenharia,documentos,timeline,riscos,copiloto}[v]||inicio)()}
...
function dashboard(){const d=S.discretionary||{},rows=S.portfolio?.municipalities||[],contracts=d.contracts||[],payments=d.payments||[],agreements=d.agreements||[];const q=(S.dashboardQuery||'').trim().toLocaleLowerCase('pt-BR');const view=rows.filter(m=>!q||[m.name,m.cnpj,m.ibge_code].join(' ').toLocaleLowerCase('pt-BR').includes(q));const table=view.map(m=>{const ibge=String(m.ibge_code),props=(d.proposals||[]).filter(r=>String(r.COD_MUNIC_IBGE)===ibge),conv=props.length?agreements.filter(a=>props.some(p=>String(p.ID_PROPOSTA)===String(a.ID_PROPOSTA))).length:0;return `<tr><td><strong>${esc(m.name)}</strong><small>${esc(m.ibge_code)}</small></td><td>${conv}</td><td>${props.length}</td><td><button data-action="open-municipality" data-value="${esc(m.ibge_code)}">Abrir município</button></td></tr>`}).join('');$('#content').innerHTML=head('Dashboard gerencial','Acompanhamento da carteira por município, contratos, convênios e execução financeira oficial.')+`<div class="toolbar"><input id="dashboardSearch" value="${esc(q)}" placeholder="Filtrar município, CNPJ ou IBGE"><button id="dashboardSearchButton">Filtrar</button></div><div class="metric-grid">${card('Municípios',rows.length,'Carteira canônica')}${card('Contratos Transferegov',contracts.length,'Fonte oficial')}${card('Convênios',agreements.length,'Fonte oficial')}${card('Pagamentos',payments.length,'Fonte oficial')}</div><div class="table-wrap"><table><thead><tr><th>Município</th><th>Convênios</th><th>Propostas</th><th>Ação</th></tr></thead><tbody>${table}</tbody></table></div>`;const apply=()=>{S.dashboardQuery=$('#dashboardSearch').value;dashboard()};$('#dashboardSearchButton').onclick=apply;$('#dashboardSearch').onkeydown=e=>{if(e.key==='Enter')apply()}}
...
const openTransferContractBase=openTransferContract;
openTransferContract=function(id){
  openTransferContractBase(id);
  const d=S.discretionary||{}, c=(d.contracts||[]).find(row=>String(row.ID_CONTRATO)===String(id));
  if(!c)return;
  const lic=(d.procurements||[]).find(row=>String(row.ID_LICITACAO)===String(c.ID_LICITACAO)), convenio=lic?.NR_CONVENIO;
  const docs=(S.integrated?.documents||[]).filter(row=>String(row.partnership_id||row.convenio||'')===String(convenio));
  const events=(S.integrated?.timeline||[]).filter(row=>String(row.entity_id||'').includes(String(convenio||id))).slice(0,30);
  const works=(S.integrated?.engineering||[]).filter(row=>(row.contracts||[]).some(x=>String(x.id_contrato||x.contract_id||'')===String(id)));
  const risks=(S.integrated?.integrity?.signals||[]).filter(row=>row.count>0);
  const host=$('#content'); if(!host)return;
  host.insertAdjacentHTML('beforeend',`<section class="operational-evidence"><h2>Evidências relacionadas</h2><div class="metric-grid">${card('Documentos',docs.length,'Relacionados por chave oficial')}${card('Obras',works.length,'Relacionadas por contrato')}${card('Eventos',events.length,'Timeline oficial')}${card('Sinais de atenção',risks.length,'Não presume irregularidade')}</div><div class="section-grid"><article class="panel"><span class="eyebrow">Documentos pesquisáveis</span>${docs.length?`<div class="cards">${docs.slice(0,20).map(row=>`<article><h3>${esc(row.number||row.document_id)}</h3><p>${esc(row.observation||'Não informado pela fonte')}</p><small>Valor: ${moneyBR(row.value)} · Situação: ${esc(row.status)}</small></article>`).join('')}</div>`:empty('Nenhum documento vinculado pela fonte')}</article><article class="panel"><span class="eyebrow">Obras e engenharia</span>${works.length?`<div class="cards">${works.map(row=>`<article><h3>${esc(row.name||row.project_id)}</h3><p>${esc(row.description||'Não informado pela fonte')}</p><small>Situação: ${esc(row.situation)} · Contratos: ${row.contract_count||0}</small></article>`).join('')}</div>`:empty('Nenhuma obra vinculada pela fonte')}</article></div><article class="panel"><span class="eyebrow">Timeline e conformidade</span>${events.length?`<div class="cards">${events.map(row=>`<article><h3>${esc(row.title)}</h3><p>${esc(row.occurred_at)} · ${esc(row.event_type)}</p><small>${esc(row.entity)} · ${esc(row.entity_id)}</small></article>`).join('')}</div>`:empty('Nenhum evento adicional vinculado')}</article><div class="notice"><strong>Leitura de risco:</strong> ${risks.length} sinal(is) oficial(is) disponível(is) no conjunto integrado. Os sinais são informativos e não constituem conclusão de irregularidade.</div></section>`)
};
renderShell=function(){
  $('#app').innerHTML=`<div class="app-shell"><aside class="app-sidebar"><div class="brand"><div class="logo">GP</div><div><strong>GovParcerias</strong><small>Intelligence</small></div></div><div class="sidebar-label">TRABALHO</div><nav>${[['inicio','Central de consulta'],['dashboard','Dashboard gerencial'],['instrumentos','Contratos e convênios'],['municipios','Municípios']].map(([id,t])=>`<button data-view="${id}" data-action="show" data-value="${id}">${t}</button>`).join('')}</nav><div class="sidebar-label">ANÁLISE</div><nav>${[['financeiro','Financeiro'],['documentos','Documentos'],['engenharia','Obras e engenharia'],['timeline','Timeline'],['riscos','Riscos e conformidade']].map(([id,t])=>`<button data-view="${id}" data-action="show" data-value="${id}">${t}</button>`).join('')}</nav><div class="sidebar-foot"><span class="status-dot"></span> Dados oficiais · carteira 121</div></aside><main class="app-main"><header class="app-header"><div><span class="header-kicker">PAINEL OPERACIONAL</span><strong>Contratos, convênios e execução pública</strong></div><div class="header-actions"><button data-action="open-palette">Ctrl K</button><button data-action="export-current">Exportar</button><button data-action="toggle-theme">${S.theme==='dark'?'Modo claro':'Modo escuro'}</button><span class="badge">${APP_VERSION}</span></div></header><section class="workspace"><div class="workspace-search"><span>⌕</span><input id="globalSearch" value="${esc(S.homeQuery||'')}" placeholder="Buscar contrato, convênio, município, CNPJ ou fornecedor"><button id="globalSearchButton">Consultar</button></div><section id="content"></section></section><footer>Fonte oficial · PNCP · Transferegov · ObrasGov · IBGE</footer></main></div><aside id="chat" class="chat"><header><strong>Copiloto verificável</strong><button data-action="close-chat">×</button></header><div class="chat-body" id="chatBody"><div class="notice">Respostas restritas aos registros oficiais sincronizados.</div></div><div class="chat-input"><input id="chatInput" placeholder="Pergunte sobre um contrato"><button data-action="assistant-query">Enviar</button></div></aside><div id="palette" class="palette"></div>`;
  $('#globalSearchButton').onclick=()=>{S.homeQuery=$('#globalSearch').value.trim();S.instrumentContractPage=1;show('instrumentos')};$('#globalSearch').onkeydown=e=>{if(e.key==='Enter')$('#globalSearchButton').click()}
};
// Dashboard por perfil: resumo executivo e fila operacional de municípios.
const dashboardBase=dashboard;
dashboard=function(){
  const d=S.discretionary||{}, mrows=S.portfolio?.municipalities||[], q=(S.dashboardQuery||'').trim().toLocaleLowerCase('pt-BR');
  const rows=mrows.filter(m=>!q||[m.name,m.cnpj,m.ibge_code].join(' ').toLocaleLowerCase('pt-BR').includes(q));
  const agreements=d.agreements||[], contracts=d.contracts||[], payments=d.payments||[], works=S.integrated?.engineering||[], docs=S.integrated?.documents||[];
  const table=rows.map(m=>{const props=(d.proposals||[]).filter(p=>String(p.COD_MUNIC_IBGE)===String(m.ibge_code));const ids=new Set(props.map(p=>String(p.ID_PROPOSTA)));const conv=agreements.filter(a=>ids.has(String(a.ID_PROPOSTA)));return `<tr><td><strong>${esc(m.name)}</strong><small>${esc(m.ibge_code)}</small></td><td>${conv.length}</td><td>${props.length}</td><td>${conv.filter(a=>String(a.SIT_CONVENIO||'').toLocaleLowerCase().includes('ativo')).length}</td><td><button data-action="open-municipality" data-value="${esc(m.ibge_code)}">Abrir visão municipal</button></td></tr>`}).join('');
  $('#content').innerHTML=head('Dashboard gerencial','Uma visão executiva para gestores e uma fila operacional para equipes de contratos, engenharia e acompanhamento municipal.')+`<div class="dashboard-intro"><div><span class="eyebrow">VISÃO MACRO · CARTEIRA OFICIAL</span><h2>O que exige atenção hoje?</h2><p>Use os indicadores para localizar rapidamente municípios, contratos e execução. Os números são calculados apenas com os registros oficiais sincronizados.</p></div><div class="dashboard-actions"><button data-action="show" data-value="instrumentos">Consultar contratos</button><button data-action="show" data-value="financeiro">Abrir financeiro</button></div></div><div class="metric-grid">${card('Municípios na carteira',mrows.length,'Planilha canônica')}${card('Convênios',agreements.length,'Transferegov')}${card('Contratos',contracts.length,'Transferegov')}${card('Pagamentos',payments.length,'Transferegov')}${card('Obras',works.length,'ObrasGov')}${card('Documentos',docs.length,'Registros oficiais')}</div><div class="toolbar"><input id="dashboardSearch" value="${esc(q)}" placeholder="Filtrar município, CNPJ ou código IBGE"><button id="dashboardSearchButton">Filtrar carteira</button></div><div class="table-wrap"><table><thead><tr><th>Município</th><th>Convênios</th><th>Propostas</th><th>Ativos</th><th>Ação</th></tr></thead><tbody>${table||`<tr><td colspan="5">Nenhum município encontrado na carteira oficial.</td></tr>`}</tbody></table></div>`;
  const apply=()=>{S.dashboardQuery=$('#dashboardSearch').value;dashboard()};$('#dashboardSearchButton').onclick=apply;$('#dashboardSearch').onkeydown=e=>{if(e.key==='Enter')apply()}
};
